...
Оба пакета собираются под `net9.0` и `net10.0`. Приложению достаточно любой из этих версий, переезд на .NET 10 ради подключения библиотеки не требуется. Публичный API на обеих ветках одинаков.
